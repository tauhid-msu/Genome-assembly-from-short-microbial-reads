...
def main():
    ...
    redQseqs = []; #np.empty(len(allolaplists), dtype='object')
    redQlabels = []; #np.empty(len(allolaplists), dtype='object')
    
   # print('allqlabels',len(allqlabels));
    
    for i in range(len(allolaplists)):
        if len(allolaplists[i])>0:
            #print(allolaplists[i]);
            rqPos = allolaplists[i][1];
            if seqlabels[rqPos] not in redQlabels:
                redQlabels.append(seqlabels[rqPos]);
                redQseqs.append(seqarray[rqPos]);          
                #print(seqlabels[rqPos]);
            if seqlabels[rqPos] not in allqlabels:
                #print(i,seqlabels[rqPos]);
                allqlabels.append(seqlabels[rqPos]);
                allqseqs.append(seqarray[rqPos]);          
                #print(seqlabels[rqPos]);
    '''
    #print('newredqlabels',len(redQlabels));         
    #print('allqlabels',len(allqlabels));
    newQseqs = addNewQseqs(stPos);
    print(len(newQseqs)); 
    '''
    redQseqarray = np.empty(len(redQseqs), dtype='object');
    #redQlabelarray =  np.empty(len(redQseqs), dtype='object'); 
    for i in range(len(redQseqs)):
        redQseqarray[i] = redQseqs[i];
        
    #redQseqs = np.asarray(redQseqs);
    #redQlabels = np.asarray(redQlabels); 
    #print(redQseqs.shape);        
    '''
